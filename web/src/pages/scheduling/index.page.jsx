...
const CardWithSchedule = ({ card, addSchedule }) => {
  const {
    ...
    error,
    isLoading,
  } = useGetScheduleByIdQuery(card.id)
  ...
  return (
    <Template>
      <div className='flex justify-between items-center mt-4 mb-4'>
        <div>
          <button
            className={`bg-blue-500 text-white px-4 py-2 rounded-md ${schedulingMode ? 'bg-opacity-100 cursor-not-allowed' : ''}`}
      ...
      <div className='grid grid-cols-3 gap-4'>
        ...
        <div
          className='bg-gray p-4 flex justify-center items-center cursor-pointer'
          onClick={addCard}
        >
          <svg
